...
const DEPRECATED_VERSIONS = ['1.0', '0.3', '0.2', '0.1']

const unmaintainedVersion = computed(() => {
  const match = route.path.match(/^\/(\d+\.\d+)(\/|$)/)
  return match && DEPRECATED_VERSIONS.includes(match[1]) ? match[1] : null
})

// VersionSelect hides itself on the root path — mirror that here so the
// divider before it doesn't render alone.
const showVersionSelect = computed(() => route.path !== '/')
  ...
  <Layout>
    ...
    <template #nav-bar-content-after>
      <div class="vp-nav-extra notranslate" translate="no">
        <span class="vp-nav-divider" aria-hidden="true" />
        <LanguageSelect />
        <span class="vp-nav-divider" aria-hidden="true" />
        <a
        ...
        <template v-if="showVersionSelect">
          <span class="vp-nav-divider" aria-hidden="true" />
...
/* Vertical divider line between the icons (matches VitePress
   .VPNavBarSocialLinks divider treatment). */
.vp-nav-divider {
...
/* Collapse any pair of adjacent dividers (defensive — if a sibling
   between them renders nothing, we should not show two lines in a row). */
.vp-nav-divider + .vp-nav-divider {
  display: none;
}
