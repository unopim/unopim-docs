...
- **Critical Bug Fixes**: If you're fixing a critical bug, also port the fix to the current stable release branch so it can ship in the next patch release.
...
UnoPim ships with **33 supported locales**. Every user-facing string (labels, messages, errors, placeholders, titles) must use Laravel's translation helper — `trans('package::file.key')` or `__('package::file.key')` — never a hardcoded string. This is enforced in CI; pull requests that add hardcoded strings or partial locale coverage will fail the translation check and cannot be merged.

### Workflow

When you add or update a translation key:

1. **Add the key in `en_US` first** — this is the source locale. Translation files live in each package's `Resources/lang/{locale}/` directory.
2. **Propagate to all 33 locales** — translate the value into the actual target language. Do not copy the English value as-is into other locales.
3. **Preserve placeholders** — Parameters such as `:attribute`, `:count`, and `:name` must remain exactly as-is in every locale. Do not translate placeholder names.
4. **Translate naturally** — Each value must read naturally in the target language, not be a literal word-for-word translation.

### Supported Locales

```
ar_AE  ca_ES  da_DK  de_DE  en_AU  en_GB  en_NZ  en_US
es_ES  es_VE  fi_FI  fr_FR  hi_IN  hr_HR  id_ID  it_IT
ja_JP  ko_KR  mn_MN  nl_NL  no_NO  pl_PL  pt_BR  pt_PT
ro_RO  ru_RU  sv_SE  tl_PH  tr_TR  uk_UA  vi_VN  zh_CN
zh_TW
```

### Translation Check (CI)

Before submitting a pull request, run the translation check command and confirm it passes with **zero errors**:
