...
### Install PostgreSQL Driver Extensions

UnoPim needs the PostgreSQL PDO extension to connect to the database.
...
**macOS** (Homebrew `php` ships `pdo_pgsql` enabled — no extra step needed).

### Create the Database and Role

Connect to PostgreSQL as a superuser and create a dedicated role and database for UnoPim:
...
```sql
CREATE ROLE unopim WITH LOGIN PASSWORD 'your_secure_password';
CREATE DATABASE unopim OWNER unopim ENCODING 'UTF8' TEMPLATE template0;
GRANT ALL PRIVILEGES ON DATABASE unopim TO unopim;
\q
```

::: tip
The default PostgreSQL port is **5432**. If you changed the port in `postgresql.conf`, supply the same value during the installer prompts or in `.env`.
:::
...
::: warning Note
- Make sure your PHP version is 8.4.1 or higher and all required PHP extensions are installed including the `pdo_pgsql` extension required for PostgreSQL.
